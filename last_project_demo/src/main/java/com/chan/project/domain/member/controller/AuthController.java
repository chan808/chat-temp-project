...
import com.chan.project.domain.member.dto.request.*;
import com.chan.project.domain.member.service.AuthService;
import com.chan.project.domain.member.service.MemberService;
import com.chan.project.global.exception.ErrorResponse;
import com.chan.project.global.mail.MailService;
import com.chan.project.global.util.CookieUtil;

import static com.chan.project.global.security.jwt.JwtContents.*;
...
public class AuthController {
    ...
    @Operation(
            summary = "[DEV] 개발용 임시 로그인 API",
            description = "개발/테스트용 임시 로그인 API입니다. 요청 시 지정된 이메일(기본값: test1@test.com)로 즉시 토큰을 발급합니다.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "로그인 성공")
            }
    )
    @GetMapping("/dev-login")
    public ResponseEntity<Void> devLogin(@RequestParam(defaultValue = "test1@test.com") String email, HttpServletResponse response) {
        AuthTokensDTO tokens = authService.devLogin(email);
    ...
    @Operation(summary = "[DEV] 비밀번호 암호화 테스트 API")
    @GetMapping("/encrypt")
    public ResponseEntity<String> encryptPassword(@RequestParam String password) {
        String encodedPassword = passwordEncoder.encode(password);
        return ResponseEntity.ok("Encoded result for '" + password + "': " + encodedPassword);
    }
}
